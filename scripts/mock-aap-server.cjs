 ...
 */
function generatePaginatedResponse(resourceType, page = 1, pageSize = 25) {
  const results = [];
 ...
 */
function generateMockResource(resourceType, id = 1) {
  const baseResource = {
    id: id,
    type: resourceType,
 ...
 */
function extractResourceType(pathname) {
  const patterns = [
    /\/api\/controller\/v2\/([^/]+)/,
    /\/api\/v2\/([^/]+)/,
    /\/api\/gateway\/v1\/([^/]+)/,
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      ...
      let resourceType = match[1].replace(/_/g, "_");
 ...
 */
function isDetailEndpoint(pathname) {
...
const server = http.createServer((req, res) => {
  ...
  if (resourceType) {
    ...
    if (req.method === "GET") {
      if (isDetailEndpoint(pathname)) {
        ...
        console.log(`${logPrefix} - Returning mock ${resourceType} detail (id=${id})`);
        res.writeHead(200, { "Content-Type": "application/json" });
        ...
        const pageSize = parseInt(parsedUrl.query.page_size || "25", 10);
        console.log(`${logPrefix} - Returning mock ${resourceType} list (page=${page})`);
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(generatePaginatedResponse(resourceType, page, pageSize)));
      }
      return;
    }
  ...
  if (pathname === "/api/" || pathname === "/api/v2/" || pathname === "/api/controller/v2/") {
  ...
  console.log(`${logPrefix} - Not implemented (returning 200 with empty result)`);
  res.writeHead(200, { "Content-Type": "application/json" });
  res.end(JSON.stringify({
