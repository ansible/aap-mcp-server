...
export class PseudoIdentityService {
   ...
   */
  getInstallerPseudoId(): string | null {
   ...
   * Returns null if the salt key is not loaded.
   * @param userInfo - User info extracted from /me/ endpoint
   */
  deriveIdentity(userInfo: UserInfo): PseudoIdentityResult | null {
    if (!this.hmacKey) {
      return null;
    }
    if (!userInfo.ansibleId) {
      return {
        userPseudoId: "anonymous",
        userType: this.determineUserType(userInfo.email),
        installerPseudoId: this.getInstallerPseudoId() || "unknown",
      };
    }
    return {
      userPseudoId: this.generatePseudoId(userInfo.ansibleId),
      userType: this.determineUserType(userInfo.email),
      installerPseudoId: this.getInstallerPseudoId() || "unknown",
    };
  }
