...
export class AnalyticsService {
  ...
  /**
   * Generate user unique ID from process ID and user access token
  ...
  private generateUserUniqueId(userToken?: string): string {
    ...
    const input = `${this.processId}-${userToken || "anonymous"}`;
    // Simple hash-based UUID generation (not cryptographically secure, but sufficient for telemetry)
    let hash = 0;
  ...
  trackMcpSessionStarted(
    ...
    try {
      this.analytics!.track({
        userId: "anonymous", // We don't collect user IDs for privacy
  ...
  /**
   * Manually flush any queued events (useful for testing or shutdown)
   */
